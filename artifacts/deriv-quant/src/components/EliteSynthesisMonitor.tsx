...
import { Loader2, FlaskConical, XCircle, CheckCircle, Square } from "lucide-react";
import {
...
export function EliteSynthesisMonitor() {
  ...
  return (
    <div className="fixed bottom-4 right-4 z-[60] w-[340px] rounded-xl border border-cyan-500/25 bg-slate-950/95 shadow-2xl backdrop-blur p-3 space-y-3">
        ...
        <>
          <div className="space-y-1">
            <div className="flex items-center justify-between text-[11px] text-muted-foreground">
              ...
              <span className="font-mono text-cyan-200">{Math.max(0, Math.min(100, progress.progressPct))}%</span>
            </div>
            <div className="h-2 overflow-hidden rounded bg-background/80">
              <div className="h-full bg-cyan-400 transition-all duration-300" style={{ width: `${Math.max(0, Math.min(100, progress.progressPct))}%` }} />
            </div>
          </div>

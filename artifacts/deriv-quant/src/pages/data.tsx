...
import {
  ...
  Download, ChevronRight, Cpu, Sparkles, ChevronDown, ChevronUp, Clock,
} from "lucide-react";
...
const RESEARCH_DATA_ACTION_HISTORY_KEY = "deriv_research_data_action_history_v1";

type DataActionHistoryEntry = {
  id: string;
  service: string;
  action: string;
  status: string;
...
function readDataActionHistory(service: string, action: string): DataActionHistoryEntry[] {
  try {
    const raw = localStorage.getItem(RESEARCH_DATA_ACTION_HISTORY_KEY);
    if (!raw) return [];
    const parsed = JSON.parse(raw) as DataActionHistoryEntry[];
...
function appendDataActionHistory(entry: DataActionHistoryEntry): DataActionHistoryEntry[] {
  try {
    const raw = localStorage.getItem(RESEARCH_DATA_ACTION_HISTORY_KEY);
    const parsed = raw ? JSON.parse(raw) : [];
    const existing = Array.isArray(parsed) ? (parsed as DataActionHistoryEntry[]) : [];
    const next = [entry, ...existing].slice(0, 40);
    localStorage.setItem(RESEARCH_DATA_ACTION_HISTORY_KEY, JSON.stringify(next));
    return next;
  } catch {
    return [entry];
  }
...
function RunHistoryPanel({
  ...
  entries: DataActionHistoryEntry[];
  expanded: boolean;
  onToggle: () => void;
  emptyMessage: string;
}) {
    ...
    <div className="rounded-lg border border-border/30 bg-background/40 p-3 space-y-3">
      <div className="flex items-center justify-between gap-2 flex-wrap">
        <div className="flex items-center gap-2">
          ...
          {entries.length > 0 && (
            <span className="text-[11px] text-muted-foreground">({entries.length} runs)</span>
          )}
        </div>
        <button
          type="button"
              ...
              <table className="w-full text-[11px]">
                ...
                <tbody>
                  {entries.slice(0, 8).map((entry) => (
                    <tr key={entry.id} className="border-b border-border/10 last:border-b-0">
                      <td className="py-2 pr-3 text-muted-foreground">{new Date(entry.at).toLocaleString()}</td>
...
function LockedSymbolBadge({ symbol }: { symbol: string }) {
  return (
    <div className="inline-flex items-center gap-2 rounded-lg border border-border/40 bg-background px-3 py-2 text-xs">
      ...
      <span className="text-muted-foreground">â€” {getSymbolLabel(symbol)}</span>
      <span className="text-[10px] text-primary">current service</span>
    </div>
  );
}

// â”€â”€ Stream State Chip â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€ Coverage Status Chip â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€ Symbol State Row â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  return (
    <tr className="border-b border-border/20 last:border-0">
      ...
      <td className="py-2.5 px-3 tabular-nums text-xs text-muted-foreground">
        {coverage?.count1m ? coverage.count1m.toLocaleString() : "â€”"}
      </td>
      <td className="py-2.5 px-3 tabular-nums text-xs text-muted-foreground">
        {coverage?.count5m ? coverage.count5m.toLocaleString() : "â€”"}
      </td>
      <td className="py-2.5 px-3 text-[11px] text-muted-foreground">
        {coverage ? formatAge(coverage.newestDate) : "â€”"}
      </td>
      <td className="py-2.5 px-4">
...
// â”€â”€ Coverage Table â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
            ...
            return (
              <tr key={sym.symbol} className="border-b border-border/20 hover:bg-muted/10">
                ...
                <td className="py-2.5 px-3 text-right tabular-nums">
                  {sym.count1m > 0 ? <span className="text-foreground font-medium">{sym.count1m.toLocaleString()}</span> : <span className="text-muted-foreground/40">â€”</span>}
                </td>
                <td className="py-2.5 px-3 text-right tabular-nums">
                  {sym.count5m > 0 ? <span className="text-foreground font-medium">{sym.count5m.toLocaleString()}</span> : <span className="text-muted-foreground/40">â€”</span>}
                </td>
                <td className="py-2.5 px-3 text-right tabular-nums text-muted-foreground">
                  {sym.totalCandles > 0 ? sym.totalCandles.toLocaleString() : "â€”"}
                </td>
...
// â”€â”€ Integrity Summary â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€ Data Operations Tab â€” Unified Canonical Cleanup â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
export function CleanCanonicalTab({
  ...
  const [historyEntries, setHistoryEntries] = useState<DataActionHistoryEntry[]>([]);
  ...
  useEffect(() => {
    setHistoryEntries(readDataActionHistory(lockedSymbol ?? symbol, "clean-canonical"));
  }, [lockedSymbol, symbol]);
  ...
  const run = async () => {
    ...
    try {
      ...
      setHistoryEntries(appendDataActionHistory({
        id: `clean-${symbol}-${Date.now()}`,
        service: symbol,
        action: "clean-canonical",
        status: "queued",
        detail: d.message ?? "Canonical clean launched in background.",
        at: new Date().toISOString(),
      }).filter((entry) => entry.service === symbol && entry.action === "clean-canonical"));
    } catch (e: any) {
      ...
      <div className="rounded-xl border border-primary/20 bg-card p-5 space-y-4">
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-0.5 leading-relaxed max-w-xl">
              Runs the full pipeline: detect &amp; fill gaps from API â†’ re-check existing interpolated
              candles and replace with real data â†’ derive all timeframes. Interpolation is only used
        ...
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <button onClick={run} disabled={running}
            ...
            {running
              ? <><Loader2 className="w-4 h-4 animate-spin" /> Cleaningâ€¦ (may take 2â€“5 min)</>
        ...
        {running && (
          <div className="rounded-lg bg-muted/20 border border-border/30 px-4 py-3 text-xs text-muted-foreground space-y-1">
            <p className="font-medium text-foreground">Starting pipelineâ€¦</p>
        ...
        {result && !err && result.started && (
          <div className="space-y-2">
            ...
            <div className="rounded-lg bg-muted/20 border border-border/30 px-4 py-3 text-xs text-muted-foreground space-y-1.5">
              <p className="font-medium text-foreground">Running in background â€” typically 2â€“5 min</p>
              <p>1. Detecting and filling gaps from Deriv API</p>
              <p>2. Re-checking interpolated candles â†’ replacing with real API data</p>
              <p>3. Running multi-timeframe enrichment (5m â†’ 1d)</p>
        ...
        <RunHistoryPanel
          ...
          emptyMessage={`No canonical clean runs recorded yet for ${lockedSymbol ?? symbol}.`}
        />
      </div>
      ...
      <div className="rounded-xl border border-border/40 overflow-hidden">
        <button
          ...
          <span className="flex items-center gap-1.5">
            <Wrench className="w-3.5 h-3.5" /> Advanced â€” Individual Operations
          </span>
...
export function HistoricalDownloadCard({
  ...
  useEffect(() => {
    setHistoryEntries(readDataActionHistory(lockedSymbol ?? symbol, "historical-download"));
  }, [lockedSymbol, symbol]);
  ...
  const startDownload = async () => {
    ...
    try {
      ...
      setHistoryEntries(appendDataActionHistory({
        id: `download-${symbol}-${Date.now()}`,
        service: symbol,
        action: "historical-download",
        status: "queued",
        detail: d.message ?? `Historical download started for ${symbol}`,
        at: new Date().toISOString(),
      }).filter((entry) => entry.service === symbol && entry.action === "historical-download"));
    } catch (e) {
      const message = (e as Error).message;
      setErr(message);
      setHistoryEntries(appendDataActionHistory({
  ...
  const resetTracker = async () => {
    ...
    try {
      ...
      setHistoryEntries(appendDataActionHistory({
        id: `download-reset-${symbol}-${Date.now()}`,
        service: symbol,
        action: "historical-download",
        status: "completed",
        detail: d.message ?? `Cleared stale tracker for ${symbol}`,
        at: new Date().toISOString(),
      }).filter((entry) => entry.service === symbol && entry.action === "historical-download"));
    } catch (e) {
      const message = (e as Error).message;
  ...
  return (
    <div className="rounded-xl border border-border/50 bg-card p-5 space-y-4">
      ...
      <div className="flex items-center gap-3 flex-wrap">
        ...
        {selectedStatus && (
          <span className="text-[11px] text-muted-foreground">
            ...
            {selectedStatus.newestDate ? ` Â· latest ${formatAge(selectedStatus.newestDate)}` : ""}
          </span>
        )}
      </div>
      {err && <ErrorBox msg={err} />}
      ...
      <RunHistoryPanel
        ...
        expanded={historyExpanded}
        onToggle={() => setHistoryExpanded((value) => !value)}
        emptyMessage={`No historical download runs recorded yet for ${lockedSymbol ?? symbol}.`}
      />
    </div>
  );
}
      ...
      <div className="rounded-xl border border-border/50 bg-card p-4 space-y-4">
        <div>
          <h3 className="text-sm font-semibold flex items-center gap-2">
            <Download className="w-4 h-4 text-primary" /> Export Candle Data â€” ZIP
          </h3>
          <p className="text-xs text-muted-foreground leading-relaxed mt-0.5">
            ...
            ZIP is the only export format â€” no CSV-only or JSON-only options.
          </p>
        </div>
        ...
        {rangeLoading && (
          <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
            <Loader2 className="w-3 h-3 animate-spin" /> Loading available rangeâ€¦
          </div>
        )}
        {range && !rangeLoading && (
          <div className="rounded-lg bg-muted/20 border border-border/30 px-3 py-2.5 space-y-1.5">
            <p className="text-[10px] text-muted-foreground uppercase tracking-wider">
              Available Range â€” {symbol}/{timeframe}
            </p>
            <div className="flex flex-wrap gap-x-6 gap-y-1">
              <span className="text-xs">
                <span className="text-muted-foreground">First: </span>
                <span className="font-mono text-foreground">{range.firstAvailableDate ?? "â€”"}</span>
              </span>
              <span className="text-xs">
                <span className="text-muted-foreground">Last: </span>
                <span className="font-mono text-foreground">{range.lastAvailableDate ?? "â€”"}</span>
              </span>
              <span className="text-xs">
                <span className="text-muted-foreground">Total: </span>
                <span className="font-mono text-foreground">{range.totalRows?.toLocaleString() ?? "â€”"} rows</span>
              </span>
              <span className="text-xs">
                <span className="text-muted-foreground">Interpolated: </span>
                <span className="font-mono text-foreground">{range.interpolatedCount?.toLocaleString() ?? "â€”"}</span>
              </span>
              <span className={cn("text-xs font-semibold",
                range.health === "ok" ? "text-green-400" : range.health === "partial" ? "text-amber-400" : "text-red-400")}>
                {range.health?.toUpperCase() ?? "â€”"}
              </span>
            </div>
          </div>
        )}
        ...
        {!noData && (
          <div className="flex items-center gap-3 flex-wrap pt-1">
            ...
            <button onClick={runExport} disabled={!canAct || exporting}
              ...
              {exporting ? "Preparing ZIPâ€¦" : "Download ZIP"}
            </button>
          </div>
        )}
        ...
        {exportOk    && <SuccessBox msg="ZIP download started â€” check your downloads folder." />}
      </div>
      ...
      {precheck && (
        <div className="rounded-xl border border-border/50 bg-card p-4 space-y-4">
            ...
            <>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  {[
                    ...
                    { label: "Date Range",   value: precheck.selectedRange?.firstDate && precheck.selectedRange?.lastDate
                      ? `${precheck.selectedRange.firstDate} â†’ ${precheck.selectedRange.lastDate}` : "â€”" },
                  ].map(({ label, value }) => (
              ...
              <div>
                <p className="text-[10px] text-muted-foreground uppercase tracking-wider mb-2">
                  Total Available â€” {symbol}/{timeframe}
                </p>
          ...
          <p className="text-[10px] text-muted-foreground border-t border-border/20 pt-3">
            ZIP contains: raw candle chunks (25k rows/file) Â· <span className="font-mono">manifest.json</span> (file map + counts) Â· <span className="font-mono">validation.json</span> (gap + interpolation audit)
          </p>
        </div>
      )}
    </div>
...
// â”€â”€ Runtime Tab (operational system state) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
              ...
              {(f as any).error ? <ErrorBox msg={(f as any).error} /> : (
                <div className="grid grid-cols-2 md:grid-cols-3 gap-0">
                  {Object.entries(f as Record<string,any>).filter(([k]) => !["symbol","error"].includes(k)).slice(0, 24).map(([k, v]) => (
                    <RuntimeKV
                      key={k}
                      k={FEATURE_LABELS[k] ? `${k} Â· ${FEATURE_LABELS[k]}` : k}
                      v={String(v ?? "â€”")}
                      mono
                    />
                  ))}
...
// â”€â”€ Coverage All Grid â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
export function CoverageAllGrid() {
  ...
  if (isLoading) return <div className="text-center py-12 text-sm text-muted-foreground">Loading multi-timeframe coverageâ€¦</div>;
      ...
      <div className="px-4 py-3 border-b border-border/30 flex items-center justify-between">
        <div>
          ...
          <p className="text-[11px] text-muted-foreground mt-0.5">
            All {symbolsWithData.length} symbols Â· {ALL_TIMEFRAMES.length} timeframes Â·
            <span className="ml-1 text-green-400">â– </span> present
            <span className="ml-2 text-amber-400">â– </span> &gt;30% interpolated
            <span className="ml-2 text-muted-foreground/30">â– </span> absent
          </p>
        </div>
        ...
        <div className="space-y-4">
          ...
          {liveSubtab === "ticks" && (
            <div className="rounded-xl border border-border/50 bg-card overflow-hidden">
              <div className="px-4 py-3 border-b border-border/30">
                <h2 className="text-sm font-semibold flex items-center gap-2">
                  <Activity className="w-4 h-4 text-primary" /> Live Ticks â€” {symbol}
                </h2>
              </div>
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
          ...
          {liveSubtab === "candles" && (
            <div className="rounded-xl border border-border/50 bg-card overflow-hidden">
              <div className="px-4 py-3 border-b border-border/30 flex items-center justify-between">
                <h2 className="text-sm font-semibold flex items-center gap-2">
                  <TrendingUp className="w-4 h-4 text-primary" /> M1 Candles â€” {symbol}
                </h2>
          ...
          {liveSubtab === "spikes" && (
            <div className="rounded-xl border border-border/50 bg-card overflow-hidden">
              <div className="px-4 py-3 border-b border-border/30">
                <h2 className="text-sm font-semibold flex items-center gap-2">
                  <Layers className="w-4 h-4 text-primary" /> Spike Events â€” {symbol}
                </h2>
                <p className="text-[11px] text-muted-foreground mt-0.5">
